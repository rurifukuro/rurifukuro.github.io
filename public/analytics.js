...
(function () {
  ...
  var DEV_KEY = 'ba_dev';
  var PROD_HOST = 'rurifukuro.github.io'; /* ここ以外（localhost の dev サーバー等）は常に開発者扱い */
  ...
  /* 🔴 本番ホスト以外（localhost の dev サーバー・ビルドのプレビュー）は無条件で開発者扱いにする。
   *    localStorage は**オリジンごとに別物**なので、本番で ?dev=1 を登録しても localhost には効かない。
   *    実被弾＝2026-08-24。未公開記事をローカルで表示確認したぶんが is_dev = false で 7 件記録され、
   *    ダッシュボード上は「公開していない記事が閲覧されている」ように見えた。
   *    記録自体は止めない（送る）＝計測経路がローカルでも動くことを確かめられる状態を保つ。 */
  if (window.location.hostname !== PROD_HOST) isDev = true;

  /* 🔴 自動操作ブラウザ（AI エージェントの内部ブラウザ・Headless・WebDriver）は無条件で開発者扱いにする。
   *
   *    真因＝`localStorage` は**ブラウザごと・オリジンごとに別物**なので、上の `?dev=1` の印は
   *    「そのブラウザで一度踏んだ」ときしか効かない。新しいブラウザで本番サイトを開くと素通りする。
   *    実被弾＝2026-09-02。作業用の内部ブラウザで本番ポータルを開いたぶんが is_dev = false で記録された。
   *    さらに悪いことに、`blog_events` は UA も IP も保存していない（プライバシー配慮）ので、
   *    **後から「どれが内部ブラウザ由来か」を機械的に判別できない**＝混ざったら取り返しがつかない。
   *    👉 だから「見る前に ?dev=1 を踏む」という**人間の記憶に頼る運用をやめて、その場で見分ける**。
   *
   *    3つ重ねているのは、1つでは必ず抜けるため（実測 2026-09-02・内部ブラウザ Claude/1.40609.0）:
   *      a. navigator.webdriver … WebDriver 経由の自動操作。内部ブラウザでは **false だった**（単独では効かない）
   *      b. UA の Claude/ トークン … 内部ブラウザの実測での決め手。他社エージェントは拾えない
   *      c. UA の Headless … ヘッドレス Chrome 全般
   *    どれか1つでも当たれば開発者扱い。記録自体は止めない（送る）＝経路が動くことを確かめられる状態を保つ。 */
  try {
    ...
    /* navigator が読めない環境では何もしない＝通常の閲覧として数える（上の印の判定は生きている） */
  }
