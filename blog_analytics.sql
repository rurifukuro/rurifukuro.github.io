...
-- ============================================================
-- 🔴 2026-09-01 データ是正: 本日（JST）の とれはんっ！LP 閲覧は開発者本人の確認分
--
--  ユーザー申告（2026-09-01・逐語）:
--    「あと今日とれはんっ！の紹介画面開いたの私なので、本日含めて同じアクセスからのものは集計から引いておいて」
--
--  🔴 blog_events は IP を持たない＝「同じアクセス」を後から突き止められる列が無い
--     （session_id は開くたびに変わる／device_type と referrer だけでは他人と区別できない）。
--     そこで**日付と対象ページで範囲を切る**＝ 2026-09-01(JST) の `/torehan/` だけを対象にする。
--     同じ日の `/urehan/`・記事ページ・他の LP は触らない。
--  🔴 行は 1 つも消さない（印を付けるだけ）＝後から「含めて見る」に切り替えられる。
--  🔴 日付が固定なので、このファイルを何度再適用しても同じ結果になる（冪等）。
--  ⚠ 以後の分は `?dev=1` の仕組み（localStorage に開発者印を持つ）が担当する。
--     この節を「今日」という相対日付に書き換えないこと＝再適用のたびに範囲が伸びる。
-- ============================================================
UPDATE blog_events
   SET is_dev = true
 WHERE page_path = '/torehan/'
   AND created_at >= timestamptz '2026-09-01 00:00:00+09'
   AND created_at <  timestamptz '2026-09-02 00:00:00+09'
   AND is_dev = false;
